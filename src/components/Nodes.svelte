<script>
    ...
    import { p1_nodes } from "../data/p1_nodes";
    import { p1_edges } from "../data/p1_edges";

    import { sol_nodes } from "../data/sol_nodes";
    import { sol_edges } from "../data/sol_edges";

    import { p1_sol_nodes } from "../data/p1_nodes_sol";
    import { p1_sol_edges } from "../data/p1_edges_sol";
    ...
    function updateRectanglePosition(selectedNode) {
        ...
        const connected_edge = edges.edges.find(
            (e) => e.source == sourceNode.ig && e.target == selectedNode.id,
        );
    ...
    // Function to update position of rectangle based on selected node
    function updateRectanglePosition_p1(selectedNode) {
        //console.log(selectedNode)
        const edges_lst = p1_edges.edges.filter(
            (e) => e.source === selectedNode.id || e.target === selectedNode.id,
        );
        if (!edges_lst) return; // No edge found
        ...
        const sourceNode = currNode;
        const connected_edge = p1_edges.edges.find(
            (e) => e.source == sourceNode.ig && e.target == selectedNode.id,
        );
    ...
    >
        ...
        {#if index === 1}
            ...
            {#each edges.edges as e}
                ...
                <polyline
                    ...
                    String(x(nodes.nodes[e.source].x)) +
                        "," +
                        String(y(nodes.nodes[e.source].y)) +
                        " " +
                        String(x(nodes.nodes[e.target].x)) +
                        "," +
                        String(y(nodes.nodes[e.target].y))}
                    stroke={e.color}
                    stroke-width="3"
                    in:draw|global={{
                        intro: true,
                ...
                <text
                    x={x(
                        (nodes.nodes[e.source].x + nodes.nodes[e.target].x) / 2,
                    ) + e.x_shift}
                    y={y(
                        (nodes.nodes[e.source].y + nodes.nodes[e.target].y) / 2,
                    ) + e.y_shift}
                >
                    {e.weight}
                </text>
        ...
        {#if index === 2}
            {#each sol_edges.edges as e}
                <polyline
                    points={// String of coordintates x0,y0 x1,y1
                    String(x(sol_nodes.nodes[e.source].x)) +
                        "," +
                        String(y(sol_nodes.nodes[e.source].y)) +
                        " " +
                        String(x(sol_nodes.nodes[e.target].x)) +
                        "," +
                        String(y(sol_nodes.nodes[e.target].y))}
                    stroke={blue}
                    stroke-width="5"
                    in:draw|global={{
                        intro: true,
        ...
        {#if index >= 5 && index < 19}
            {#if index < 7}
                <polyline
                    ...
                    String(x(nodes.nodes[0].x)) +
                        "," +
                        String(y(nodes.nodes[0].y)) +
                        " " +
                        String(x(nodes.nodes[5].x)) +
                        "," +
                        String(y(nodes.nodes[5].y))}
                    stroke={pink}
                    stroke-width="5"
                    in:draw|global={{
                        intro: true,
            ...
            />
            <text x={x(nodes.nodes[0].x) - 4} y={y(nodes.nodes[0].y) + 5}
                >a</text
            >
            <text x={x(nodes.nodes[5].x) - 4} y={y(nodes.nodes[5].y) + 5}
                >f</text
            >
            <text x={x(nodes.nodes[1].x) - 4} y={y(nodes.nodes[1].y) + 5}
                >b</text
            >
            <text x={x(nodes.nodes[2].x) - 4} y={y(nodes.nodes[2].y) + 5}
                >c</text
            >
        {/if}
        ...
        {#if index >= 6 && index < 7}
            ...
            <text
                x={x(nodes.nodes[2].x) +
                    uest_vals[nodes.nodes[2].id]["x_shift"]}
                y={y(nodes.nodes[2].y) +
            ...
            <text
                x={x(nodes.nodes[5].x) +
                    uest_vals[nodes.nodes[5].id]["x_shift"]}
                y={y(nodes.nodes[5].y) +
